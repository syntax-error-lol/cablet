    ...
    constructor(partial: Partial<PrivateUser>) {
        ...
        if (this.subscriptions) this.badges = [
            ...
            ...this.subscriptions.reduce((acc, subscription) => {
                const group = subscription?.subscription?.group;
                if (group && group.imageId !== null) {
                    return [
                        ...acc,
                        {
                            ...group,
        ...
        if (this.permissions && this.groups) this.permissions = [...new Set([
            ...
            ...this.subscriptions.reduce((acc, subscription) => [...acc, ...(subscription?.subscription?.group?.permissions ?? [])], [])
        ])];
