...
export const useFriend = create<FriendStore>((set, get) => ({
    ...
    isRequesting: (userId) => {
        ...
        return getSendingRequestsCommon({ friends, friendedBy, blocked: [] }).some((u: { id: string }) => u.id === userId);
    },

    isRequestedBy: (userId) => {
        const { friends, friendedBy } = get();

        return getFriendRequestsCommon({ friends, friendedBy, blocked: [] }).some((u: { id: string }) => u.id === userId);
    },
