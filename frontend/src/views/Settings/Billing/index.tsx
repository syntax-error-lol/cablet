...
export default function SettingsBilling() {
    ...
    useEffect(() => {
        ...
        if (primaryPaymentMethod) {
            const type = primaryPaymentMethod.type === "CARD" ? "Card" : primaryPaymentMethod.type === "CASHAPP" ? "Cash App" : primaryPaymentMethod.type === "PAYPAL" ? "PayPal" : "Payment";
            setPaymentMethodString(`${type} • ${primaryPaymentMethod.value}`);
        } else {
            setPaymentMethodString(null);
        }

        if (user.subscription) {
            const product = products.find((p) => p.id === user.subscription!.subscriptionId);
            if (!product) return;
