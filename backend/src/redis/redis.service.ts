...
export class RedisService extends Redis {
    ...
    constructor(
        ...
        const redisUrl = configService.get<string>("SERVER_REDIS_URL");
        const redisOptions = typeof redisUrl === "string" && redisUrl.length > 0 ? redisUrl : {
            host: configService.get<string>("SERVER_REDIS_HOST", "localhost"),
            port: configService.get<number>("SERVER_REDIS_PORT", 6379),
            password: configService.get<string>("SERVER_REDIS_PASSWORD")
        };

        super(redisOptions as any);
