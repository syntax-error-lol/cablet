...
export class StripeService {
    ...
    async handlePaymentIntent(event: Stripe.PaymentIntent) {
        ...
        for (let i = 0; i < (parseInt(event.metadata.quantity as string) || 1); i++) {
            ...
            if (product.groupId && !product.isSubscription) {
                ...
                const existingMembership = await this.prismaService.user.findUnique({
                    where: { id: user.id },
                    select: { groups: { where: { id: product.groupId } } }
                });
                if (!existingMembership?.groups.some((entry) => entry.id === product.groupId)) {
                    await this.prismaService.user.update({
                        where: { id: user.id },
                        data: { groups: { connect: { id: product.groupId } } }
                    });
                }
            ...
            if (product.isSubscription) await this.prismaService.$transaction(async (tx) => {
                ...
                const existingSubscription = await tx.userSubscription.findFirst({
                    where: {
                        userId: user.id,
                        subscriptionId: product.id as any,
                        status: UserSubscriptionStatus.ACTIVE,
        ...
        return await this.prismaService.$transaction(async (tx) => {
            ...
            return await tx.userPaymentMethod.create({
                data: {
                    ...
                    paymentMethodId: paymentMethod.id,
                    type: "CARD" as any,
                    value: paymentMethod.card?.last4 ?? paymentMethod.id,
                    primary: true
                } as any
            });
        });
    ...
    async createPaymentIntent(userId: string, productId: number, dto: StripeCreatePaymentIntentDto, ipAddress: string) {
        ...
        const user = await this.prismaService.user.findUnique({
            ...
            include: {
                subscriptions: {
                    where: {
                        subscriptionId: productId as any,
                        status: UserSubscriptionStatus.ACTIVE,
                        expiresAt: null
                    }
                }
    ...
    async createSubscription(userId: string, productId: number, ipAddress: string) {
        const hasSubscription = await this.prismaService.userSubscription.count({ where: { userId, subscriptionId: productId as any, status: UserSubscriptionStatus.ACTIVE } as any });
