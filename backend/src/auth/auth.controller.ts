...
import { AuthAuthEntity, AuthOtpEntity, AuthWebAuthnGenerateWebAuthnRegistrationDto, BadRequest, Forbidden, InternalServerError, NotFound } from "@blacket/types";
...
export class AuthController {
    ...
    async generateWebAuthnRegistration(
        @GetCurrentUser() userId: string,
        @Body() dto: AuthWebAuthnGenerateWebAuthnRegistrationDto
    ): Promise<any> {
        return this.authService.generateWebAuthnRegistration(userId, dto);
    }

    @Post("webauthn/verify-registration")
    async verifyWebAuthnRegistration(@GetCurrentUser() userId: string, @Body() dto: any): Promise<UserWebAuthn> {
