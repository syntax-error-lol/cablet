...
import { PrismaClient, RarityAnimationType, RewardType } from "@blacket/core";
...
export class PrismaService extends PrismaClient implements OnModuleInit {
    constructor(
        ...
        super({
            datasources: {
                db: {
                    url: configService.get<string>("SERVER_DATABASE_URL") ?? `postgresql://${configService.get("SERVER_DATABASE_USER")}:${configService.get("SERVER_DATABASE_PASSWORD")}@${configService.get("SERVER_DATABASE_HOST")}:${configService.get("SERVER_DATABASE_PORT") ?? 5432}/${configService.get("SERVER_DATABASE_NAME")}?schema=public`
                }
            },
            omit: {
                userSetting: { id: true },
    ...
    // this is used in development to quickly add some things into the db
    async dev() {
        for (const thing of [
            "Aquatic.mp3",
            "Autumn.mp3",
            ...
            let resid = await this.resource.findFirst({
                where: { path: `{cdn}/content/packs/ambiences/${thing}` }
            });
            if (!resid) {
                resid = await this.resource.create({
                    data: { path: `{cdn}/content/packs/ambiences/${thing}` }
                });
            }

            try {
                await this.pack.update({
                    where: { name: thing.replace(".mp3", "") },
                    data: { ambienceId: resid.id }
                });

                console.log(`Added ambience ${thing}`);
            } catch {
                console.log(`Failed to add ambience ${thing}, does the pack exist?`);
            }
    ...
    async seedDatabase() {
        ...
        await this.$transaction(async (tx) => {
            ...
            await tx.font.createMany({ data: [{ name: "Titan One", resourceId: defaultFont2.id, default: true, priority: 2 }] });
            await tx.title.create({ data: { name: "Normal", fontId: defaultFont.id } });
            ...
            const sw = await tx.spinnyWheel.create({ data: { name: "Daily Spinny Wheel" } });

            const tokenReward = await tx.reward.create({ data: { name: "Daily Spinny Wheel Tokens", type: RewardType.TOKENS, quantity: 500 } });
            const tokenReward2 = await tx.reward.create({ data: { name: "Daily Spinny Wheel Tokens 2", type: RewardType.TOKENS, quantity: 600 } });
            const tokenReward3 = await tx.reward.create({ data: { name: "Daily Spinny Wheel Tokens 3", type: RewardType.TOKENS, quantity: 700 } });
            const tokenReward4 = await tx.reward.create({ data: { name: "Daily Spinny Wheel Tokens 4", type: RewardType.TOKENS, quantity: 800 } });
            const tokenReward5 = await tx.reward.create({ data: { name: "Daily Spinny Wheel Tokens 5", type: RewardType.TOKENS, quantity: 900 } });
            const tokenReward6 = await tx.reward.create({ data: { name: "Daily Spinny Wheel Tokens 6", type: RewardType.TOKENS, quantity: 1000 } });
            const itemReward = await tx.reward.create({ data: { name: "Daily Spinny Wheel Ticket", type: RewardType.ITEM, itemId: swtItem.id, quantity: 1 } });
            const crystalReward = await tx.reward.create({ data: { name: "Daily Spinny Wheel Crystals", type: RewardType.CRYSTALS, quantity: 5 } });

            await tx.spinnyWheelReward.createMany({
                data: [
                    { rewardId: tokenReward.id, spinnyWheelId: sw.id, chance: 20 },
                    { rewardId: tokenReward2.id, spinnyWheelId: sw.id, chance: 20 },
                    { rewardId: tokenReward3.id, spinnyWheelId: sw.id, chance: 20 },
                    { rewardId: tokenReward4.id, spinnyWheelId: sw.id, chance: 20 },
                    { rewardId: tokenReward5.id, spinnyWheelId: sw.id, chance: 20 },
                    { rewardId: tokenReward6.id, spinnyWheelId: sw.id, chance: 20 },
                    { rewardId: itemReward.id, spinnyWheelId: sw.id, chance: 20 },
                    { rewardId: crystalReward.id, spinnyWheelId: sw.id, chance: 5 }
                ]
            });
